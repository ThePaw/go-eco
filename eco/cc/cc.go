...
func Coenocline(nSpec, nSamp int, m Models) (out *Matrix) {
	...
	srf := responseFn(m.SRF)
	points := generate_points(nSamp, m.Samp)    // generate sampling points
	rngO := rndFn(m.Opt, m.OptMean, m.OptScale) // optima distribution model
	...
	rngM := rndFn(m.Max, m.MaxLoc, m.MaxScale) // population size distribution model
	rngT := rndFn(m.Tol, m.TolLoc, m.TolScale) // tolerance distribution model
