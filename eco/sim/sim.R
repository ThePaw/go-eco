...
{	
	if (!is.na(pmatch(method, "jaccard"))) 
        method <- "jaccard"
	METHODS <- c("soerensen", "jaccard", "ochiai", "mountford", "whittaker", "lande", "wilsonshmida", "cocogaston", "magurran", "10 harrison", "11 cody", "12 williams", "13 williams2", "harte", "15 simpson", "lennon", "weiher", "ruggiero", "lennon2", "20 rout1ledge", "rout2ledge", "rout3ledge", "sokal1", "dice", 
...
40 "sokal4", "stiles", "yule", "michael", "hamann", 
45 "forbes", "chisquare", "peirce", "eyraud", "simpson2", 
50 "legendre2", "fager", "maarel", "lamont", "johnson", 
55 "sorgenfrei", "johnson2", "euclidean", "divergence")
	...
	if (method == 1) {
        ...
        dis <- 1-((a*((2*a)+b+c))/(2*(a+b)*(a+c))) ## cody
    }
    else if (method == 12) {
        dis <- pmin(b,c) / (a+b+c) 
    }
